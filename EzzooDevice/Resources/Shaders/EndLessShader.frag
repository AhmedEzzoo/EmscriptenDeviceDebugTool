...
void main()
{
    ...
    float LOD = max(0.0, log10(l * 2.0 / 0.025) + 1.0);

    float GridCellSizeLod0 = 0.025 * pow(10.0, floor(LOD));
    ...
    if (Lod2a > 0.0) {
        ...
        if (Lod1a > 0.0) {
            Color = mix(vec4(0.0, 0.0, 0.0, 1.0), vec4(0.5, 0.5, 0.5, 1.0), LOD_fade);
	        Color.a *= Lod1a;
        } else {
    ...
    float OpacityFalloff = (1.0 - satf(length(WorldPos.xz - gCameraWorldPos.xz) / 100.0));
